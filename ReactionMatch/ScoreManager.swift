...
public protocol ScoreManagerFocusDelegate: class {
...
open class ScoreManager {
    ...
    weak var focusDelegate: ScoreManagerFocusDelegate?
    ...
    fileprivate func updateLocalHighScores() {
        for gameType in [GameType.classicMode, GameType.v2Mode] {
    ...
    fileprivate func updateLocalHighScoreForGameType(_ gameType: GameType) {
        ...
        leaderboardRequest.loadScores { (_, error) in
            guard error == nil else {
                print(error!)
                return
            }
    ...
    // MARK: Leaderboards
    func getFriendsHighScoresForGameType(_ gameType: GameType,
                                         withCompletionHandler completionHandler: @escaping ([FriendsScore]) -> Void) {
